...
```{R}
baseline <- lmer(readng_scr_p0 ~ 1 + (1 | schoolid_nces_enroll_p0), data = train3)
...
```{R}
m4 <- lmer(readng_scr_p0 ~ .- replacement_id - acadyear -schoolid_nces_enroll_p0 + (1 | schoolid_nces_enroll_p0), data = train4)
```
...
```{R}
m5 <- lmer(readng_scr_p0 ~ 1 + readng_scr_m1 + (1 | schoolid_nces_enroll_p0), data = train5)
```
